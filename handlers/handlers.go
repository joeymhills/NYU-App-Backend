...
func CreateAward(db *sql.DB, c *cache.Cache) func(w http.ResponseWriter, r *http.Request) {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        log.Println("log right after unmarshall", award) 
            ...
            res, err := db.Exec("INSERT INTO accolade VALUES (id, name, institution, outcome, serviceLine, extSource, intSource, messaging, comments, frequency, notifDate, cmcontact, sourceatr, wherepubint, promotionlim, expirationDate, effectiveDate, imgurl1, imgurl2, imgurl3, imgurl4, supported, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
